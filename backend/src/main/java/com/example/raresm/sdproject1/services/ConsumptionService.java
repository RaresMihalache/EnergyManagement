...
//            backlogRepository.save(backlog);
    ...
    public boolean compareConsumptions(Consumption consumptionA, Consumption consumptionB){
        if (consumptionB.getDate().compareTo(consumptionA.getDate()) == 0
            && consumptionA.getStartHour() == consumptionB.getStartHour()
            && consumptionA.getEndHour() == consumptionB.getEndHour()){
    ...
    public void deleteComparableConsumptions(Integer deviceId, Consumption newConsumption){
        List<Consumption> consumptions = (List<Consumption>) findAll();
        for(Consumption consumption : consumptions){
            if((compareConsumptions(newConsumption, consumption)) && (!newConsumption.getId().equals(consumption.getId()))){
                deleteConsumptionById(consumption.getBacklog().getId(), consumption.getId());
            }
        }
